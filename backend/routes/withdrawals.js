...
router.post('/create', (req, res) => {
  try {
    ...
    runQuery('UPDATE users SET balance_ton = balance_ton - ? WHERE id = ?', [amount, user.id]);
    runQuery('INSERT INTO withdrawals (user_id, amount, wallet_address) VALUES (?, ?, ?)', [user.id, amount, wallet_address]);
    runQuery('INSERT INTO transactions (user_id, type, amount, description) VALUES (?, ?, ?, ?)',
...
router.get('/:telegramId', (req, res) => {
  try {
    ...
    if (!user) return res.status(404).json({ error: 'User not found' });
    const withdrawals = getAll('SELECT * FROM withdrawals WHERE user_id = ? ORDER BY created_at DESC LIMIT 50', [user.id]);
